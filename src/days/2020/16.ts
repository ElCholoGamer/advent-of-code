...
function part1WithRules(input: string[]): [number, Record<string, number[][]>] {
	...
	const result = input.reduce((acc, line, index, arr) => {
		if (bypass.indexOf(line) !== -1 || arr[index - 1] === 'your ticket:') return acc;
		...
		if (nums.some(isNaN)) {
			...
			const ranges = values.split(' or ').map(group => group.split('-').map(Number));
...
export const part2: AoCPart = input => {
	...
	const validTickets = input
		.filter((line, index, arr) => {
			...
			const nums = line.split(',').map(num => Number(num));
			if (nums.some(n => isNaN(n)) || arr[index - 1] === 'your ticket:') return false;
		...
		validTickets.forEach(nums => {
			nums.forEach((num, index) => {
				...
				if (!current.size) {
					...
					const completedSets = Object.values(fieldPositions).filter(set => set.size === 1);
	...
	const departures = Object.keys(fieldPositions)
		.filter(key =>
			fieldPositions[<number>(<unknown>key)].values().next().value.startsWith('departure')
		)
		.map(Number);

	// Get my ticket
	const selfTicket = input.find((l, index, arr) => arr[index - 1] === 'your ticket:')!.split(',');

	// Calculate result of departure fields
	return departures.map(index => selfTicket[index]).reduce((acc, num) => acc * Number(num), 1);
};
